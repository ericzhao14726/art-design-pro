      ...
      <ElCard shadow="never" class="art-table-card">
        ...
        <ElDialog v-model="dialogVisible" :title="dialogTitle" width="55%" align-center>
          <ElForm
            ...
            label-width="90px"
          >
            <ElFormItem label="功能类型" prop="productId">
            ...
            <PropertyFormItem
              v-if="formData.type === 'Property'"
              v-model="formData"
              :data-types-options="dataTypeOptions"
              :disabled="formDisabled"
            />
            <EventFormItem
...
<script setup lang="ts">
  ...
  //   DataServiceService.getProducts({
  //     name: q,
  //     pageNo: 1,
  //     perPage: 10
  //   }).then((res) => {
  ...
  const deleteFuncModel = (modelId: string) => {
    ElMessageBox.confirm('确定要删除该功能模型吗？', '删除功能模型', {
      ...
      DataServiceService.deleteFuncModel([modelId])
        .then(() => {
          getFuncModelList()
        })
        .catch((error) => {
  ...
  const getFuncModelList = async () => {
    ...
    try {
      ...
      const getFuncModelsResp = await DataServiceService.getFuncModels({
        pageNo: currentPage,
        perPage: pageSize,
        modelIds: [],
  ...
  const createFuncmodel = () => {
    DataServiceService.createFuncModel({ ...formData })
      .then(() => {
        getFuncModelList()
      })
      .catch((error) => {
  ...
  const updateFuncModel = () => {
    ...
    DataServiceService.updateFuncModel({ ...formData, _: 1 })
      .then(() => {
        getFuncModelList()
      })
      .catch((error) => {
